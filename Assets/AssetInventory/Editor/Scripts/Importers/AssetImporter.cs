    ...
    public abstract class AssetImporter : AssetProgress
    {
        ...
        protected static Asset Fetch(Asset asset)
        {
            ...
            if (asset.AssetSource == Asset.Source.RegistryPackage)
            {
                ...
                {
                    return DBAdapter.DB.Find<Asset>(a => a.Location == asset.Location && a.PackageSource == PackageSource.Local);
                }
                return DBAdapter.DB.Find<Asset>(a => a.SafeName == asset.SafeName);
            }
            if (asset.AssetSource == Asset.Source.Archive)
            {
                return DBAdapter.DB.Find<Asset>(a => a.Location == asset.Location && a.ParentId == asset.ParentId);
            }
            ...
            if (asset.ForeignId > 0 && !string.IsNullOrEmpty(asset.Location))
            {
                result = DBAdapter.DB.Table<Asset>()
                    .FirstOrDefault(a => a.ForeignId == asset.ForeignId && a.Location.ToLower() == asset.Location.ToLower() && a.ParentId == asset.ParentId);
            }
            ...
            {
                result = DBAdapter.DB.Table<Asset>()
                    .FirstOrDefault(a => a.Location.ToLower() == asset.Location.ToLower() && a.ParentId == asset.ParentId);
            }
